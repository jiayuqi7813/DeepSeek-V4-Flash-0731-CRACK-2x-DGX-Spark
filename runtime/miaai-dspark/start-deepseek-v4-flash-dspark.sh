...
DSPARK_API_KEY_FILE="${DSPARK_API_KEY_FILE:-$HOME/.config/dspark/vllm-api-key}"
CURL_AUTH_ARGS=()
if [ -s "$DSPARK_API_KEY_FILE" ]; then
  CURL_AUTH_ARGS=(-H "Authorization: Bearer $(tr -d '\r\n' <"$DSPARK_API_KEY_FILE")")
fi
...
exit 1
EOF
  remote="${remote/__DSPARK_HCA__/$(printf '%q' "$hca")}"
  remote="${remote/__DSPARK_GID_HEX__/$(printf '%q' "$hex")}"
  if [ -z "$ssh_target" ]; then
    bash -c "$remote"
  else
    # shellcheck disable=SC2029
...
print_resolved_profile() {
  ...
  echo "  MiaAI base hotfixes: ${ENABLE_MIAAI_BASE_HOTFIXES:-0}"
  echo "  Issue #22 hotfix: ${ENABLE_DSV4_ISSUE22_HOTFIX:-0}"
  echo "  Issue #26 hotfix: ${ENABLE_DSV4_ISSUE26_HOTFIX:-0}"
  echo "  vLLM #50004 hotfix: ${ENABLE_DSV4_50004_HOTFIX:-0}"
  echo "  vLLM #49897 hotfix: ${ENABLE_DSV4_49897_HOTFIX:-0}"
  echo "  MiaAI long-prefill tuning: ${ENABLE_MIAAI_LONG_PREFILL_TUNING:-0}"
...
for hotfix in "${DSV4_HOTFIX_FILES[@]}"; do
  if [ ! -f "$SCRIPT_DIR/patches/$hotfix" ]; then
    echo "Missing DSV4 hotfix: $SCRIPT_DIR/patches/$hotfix" >&2
    exit 1
  fi
done
...
for hotfix in "${DSV4_HOTFIX_FILES[@]}"; do
  scp "$SCRIPT_DIR/patches/$hotfix" "${WORKER_HOST}:${REMOTE_WORKER_DIR}/patches/$hotfix"
done
if [ "$ENABLE_VLLM_GB10_PATCH" = "1" ]; then
...
for _ in $(seq 1 "$WAIT_ATTEMPTS"); do
  if curl -fsS --max-time 5 "${CURL_AUTH_ARGS[@]}" "$API_URL" >/dev/null 2>&1; then
